...
class IsarDatasource extends LocalStorageDatasource {
  ...
  Future<bool> isMovieWatched(int movieId) async {
    ...
    final WatchedMovie? isMovieWatched =
        await isar.watchedMovies.filter().movieIdEqualTo(movieId).findFirst();

    return isMovieWatched != null;
  }
  ...
  Future<bool> isMovieInWatchlist(int movieId) async {
    ...
    final WatchlistMovie? isMovieInWatchlist =
        await isar.watchlistMovies.filter().movieIdEqualTo(movieId).findFirst();

    return isMovieInWatchlist != null;
  }
  ...
  Future<void> toggleWatched(Movie movie) async {
    ...
    final watchedMovie =
        await isar.watchedMovies.filter().movieIdEqualTo(movie.id).findFirst();

    await isar.writeTxnSync(() async {
      if (watchedMovie != null) {
        isar.watchedMovies.deleteSync(watchedMovie.isarId);
      } else {
        isar.watchedMovies.putSync(
            WatchedMovie(movieId: movie.id, watchedDate: DateTime.now()));
      }
    });
  }
  ...
  Future<void> toggleWatchlist(Movie movie) async {
    ...
    final movieInWatchlist = await isar.watchlistMovies
        .filter()
        .movieIdEqualTo(movie.id)
        .findFirst();

    await isar.writeTxnSync(() async {
      if (movieInWatchlist != null) {
        isar.watchlistMovies.deleteSync(movieInWatchlist.isarId);
      } else {
        isar.watchlistMovies.putSync(
